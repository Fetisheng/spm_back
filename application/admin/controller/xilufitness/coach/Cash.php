...
class Cash extends Backend
{
    ...
    /**
     * Cash模型对象
     * @var \app\admin\model\xilufitness\coach\Cash
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\xilufitness\coach\Cash;
        $this->assignconfig("isType",$this->model->getTypeList());
        $this->assignconfig('cashType',$this->model->getCashType());
        $this->assign("isType",$this->model->getTypeList());
        $this->assign('cashType',$this->model->getCashType());
    }
        ...
        $list = $this->model
            ...
            ->where(function ($query) use($fitness_brand_id){
                if($fitness_brand_id > 0){
                    $query->where('cash.brand_id','eq',$fitness_brand_id);
                }
            })
            ->order($sort, $order)
            ->paginate($limit);
