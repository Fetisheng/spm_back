...
class Account extends Backend
{
    ...
    /**
     * Account模型对象
     * @var \app\admin\model\xilufitness\user\Account
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\xilufitness\user\Account;
    ...
    public function index()
    {
        ...
        $list = $this->model
            ->with(['user' => function($query){
                $query->withField(['nickname','mobile']);
            ...
            ->where(function ($query) use($fitness_brand_id){
                if($fitness_brand_id > 0){
                    $query->where('account.brand_id','eq',$fitness_brand_id);
                }
            })
            ->order($sort, $order)
            ->paginate($limit);
