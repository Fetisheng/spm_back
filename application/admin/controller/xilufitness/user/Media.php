...
class Media extends Backend
{
    ...
    /**
     * Media模型对象
     * @var \app\admin\model\xilufitness\user\Media
     */
    protected $model = null;
    protected $relationSearch = true;
    protected $searchFields = 'user.nickname,brand.brand_name,medal.medal_name';
    public function _initialize()
    {
        parent::_initialize();
        $this->model = new \app\admin\model\xilufitness\user\Media;
    ...
    public function index()
    {
        ...
        if (false === $this->request->isAjax()) {
            ...
            $this->assignconfig('media_id',$this->request->param('media_id/d',0));
        ...
        $list = $this->model
            ...
            }, 'medal' => function($query){
                $query->withField(['medal_name']);
            }])
            ->where($where)
            ->where(function ($query) use($fitness_brand_id){
                if($fitness_brand_id > 0){
                    $query->where('media.brand_id','eq',$fitness_brand_id);
                }
            })
            ->order($sort, $order)
            ->paginate($limit);
