(function(global) {
    var Timetables = function(option) {
        ...
        this.merge = typeof option.merge === 'boolean' ? option.merge : false;
            ...
            timetable.forEach(function (values, index) {
                ...
                values.forEach(function (item, i) {
                    ...
                    if (merge && listMerge[i][index].length > 1) {
                        ...
                        mergeDom.innerHTML = "<p>"+listMerge[i][index].name+"</p>"+"<p>"+listMerge[i][index].name+"</p>";
                        ...
                        if (merge && listMerge[i][index].length === 0) {
                            ...
                            if (item) {
                                ...
                                if (Array.isArray(item)) {
                                    var li_html = "";
                                    if (item.length > 2) {
                                        li_html +="<i>"+item.length+"</i>";
                                    }
                                    item.forEach(function (item, i) {
                                        if (i < 2) {
                                            li_html += "<p>"+item.title+"</p>";
                                        }
                                    });
                                    courseItem.innerHTML = li_html;
                                }else {
                                    courseItem.innerHTML = "<p>"+item+"</p>" || '';
                                }
                            }else{
